  ...
  // 表示用バージョン（"1.0.<このPRの番号>"）。GitHub Pagesが最新化されたかを
  // タイトル画面で見分けられるように、この値は .github/workflows/bump-version.yml が
  // PRを開いた時点／PRブランチへのpush時点で自動更新する（手動で書き換える必要はない）。
  ...
  const game = {
    ...
    ballWaiting: false, // ミス後、パドルに乗って発射待ちの状態か（trueの間はボールが動かず、画面タップで発射する）
  ...
  // update() 側でパドルに追従させ、画面タップ（launchBall）が押されるまで動かない。
  ...
  // ==================================================================
  //  入力（タッチ・マウスなどのポインター。スマホ版で追加）
  // ==================================================================
  // 画面上の座標（CSSピクセル）を、キャンバス内部の座標（480×640）に変換する
  ...
  // 右上のポーズボタンの当たり判定領域（HUDのLIFE/MAX COMBO表示と被らない位置）
  const PAUSE_BUTTON_HIT = { x: W - 46, y: 54, w: 36, h: 36 };
  function isInPauseButton(pt) {
    return pt.x >= PAUSE_BUTTON_HIT.x && pt.x <= PAUSE_BUTTON_HIT.x + PAUSE_BUTTON_HIT.w
        && pt.y >= PAUSE_BUTTON_HIT.y && pt.y <= PAUSE_BUTTON_HIT.y + PAUSE_BUTTON_HIT.h;
  }

  // タイトル画面の「チュートリアルをはじめる」ボタンの当たり判定領域
  const TUTORIAL_BUTTON_HIT = { x: W / 2 - 80, y: H / 2 + 44, w: 160, h: 32 };
  function isInTutorialButton(pt) {
    return pt.x >= TUTORIAL_BUTTON_HIT.x && pt.x <= TUTORIAL_BUTTON_HIT.x + TUTORIAL_BUTTON_HIT.w
  ...
  canvas.addEventListener("pointerdown", (e) => {
    ...
    if (game.state === "gameover") { startNewGame(1); return; }
    if (game.state === "clear") { nextStage(); return; }

    if (game.state === "tutorial" && TUTORIAL_STEPS[game.tutorial.step].info) {
      advanceTutorialInfoStep(); // tutorial.js で定義
      return;
    }
    ...
    if (ball.y - ball.r > H) {
      if (game.state === "playing" && game.timers.star > 0) {
        ...
        if (game.lives <= 0) {
          ...
          resetPaddle();
          attachBallToPaddle(); // 即座に発射せず、画面タップで発射するまで待機する（Issue #50）
        }
      } else if (game.state === "tutorial") {
  ...
  function drawInstructions() {
    const lines = [
      ...
      "パドルに触れている間、力が溜まります",
      "→ 指を離す瞬間にブロックを2つ壊せる",
      "（発動後" + CONFIG.powerCooldownSeconds + "秒はクールダウン）",
      "右上の ❚❚ ボタンでポーズ",
      "ブロックを全部消すと次のステージへ",
      "ボールを落とすと残機が減ります",
  ...
  // スタート画面用の「チュートリアルをはじめる」ボタン（詳しい操作一覧はポーズ画面で見られる）
  function drawReadyHint() {
    const b = TUTORIAL_BUTTON_HIT;
    drawRoundRect(b.x, b.y, b.w, b.h, 8, "#1c2130");
    ...
    ctx.fillText("チュートリアルをはじめる", b.x + b.w / 2, b.y + b.h / 2 + 1);
  ...
  function draw() {
    ...
    // ポーズボタンのアイコン（❚❚。プレイ中・チュートリアル中だけタップできる）
    if (game.state === "playing" || game.state === "tutorial") {
      ctx.fillStyle = "#9aa4bb";
      ctx.fillRect(PAUSE_BUTTON_HIT.x + 8, PAUSE_BUTTON_HIT.y + 8, 6, 20);
      ctx.fillRect(PAUSE_BUTTON_HIT.x + 22, PAUSE_BUTTON_HIT.y + 8, 6, 20);
    }
    ...
    // 状態に応じた案内メッセージ
    if (game.state === "ready") {
      drawCenterText("ブロック崩し", "画面をタップで本編スタート");
      ...
      drawCenterText("ゲームオーバー", "画面をタップでもう一度");
      drawGameOverStats();
    } else if (game.state === "clear") {
      drawCenterText("ステージクリア！", "画面をタップで次のステージ");
    } else if (game.paused) {
      drawCenterText("一時停止中", "画面をタップで再開");
