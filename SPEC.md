...
- 画面表示（`drawBoss()`）: 「怖さ」重視のデザイン。
  ...
  - **シールド**: `boss.diving` の間だけ、ボスを囲む青い楕円の輪（`#78d2ff`、`shadowBlur`で発光・脈動）を描く＝無敵中であることのテレグラフ。
