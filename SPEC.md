...
- バージョン表示: `const VERSION = "1.0.<PR番号>";`（`canvas`/`ctx` 取得の直後に定義）を、`draw()` の `ready` 分岐の最後で画面右下（`x = W-8` 右寄せ, `y = H-8`）に `"v" + VERSION` として小さく（11px・グレー`#6c7486`）表示する。GitHub Pagesが最新の変更を反映しているかを見分けるための目印で、ゲームロジックには一切関与しない。PC版・モバイル版それぞれの `game.js` に同じ書式の値を持たせる。**`.github/workflows/bump-version.yml`（GitHub Actions）が、PRを開いた時点／PRブランチへのpush時点（マージを待たない）で自動更新する**（別リポジトリ `shooter` の `version-bump.yml` と同じ方式）。`pull_request: opened` またはPRブランチへの `push` で起動し、`gh pr list`/`gh pr view` でPR番号とそのPRが実際に変更したファイルを調べ、`game.js`/`mobile/game.js` のうち変更されたものだけ `VERSION` を `1.0.<そのPRの番号>` に書き換えて、**PRブランチ自体に直接**コミット・pushする（`main`ではない）。`push` トリガーは、このワークフロー自身の更新コミットで無限ループにならないよう `pusher.name != 'github-actions[bot]'` で除外している。以前は手動更新→マージ後に自動更新、と2段階の方式を試した後、shooterと運用を揃えるためこの方式に変更した。
...
- 発射待ち（`game.ballWaiting`、Issue #50）: PC版同様、ミスして残機が残っている場合はボールが
  ...
  文言を差し替えている。
- `tutorial.js` は文言をタッチ操作向けに変更（「スペースキーで次へ」→「画面をタップで次へ」、
  「Shiftキーを押しっぱなしに」→「画面に触れたままに」、「Pキーでポーズ」→「❚❚ボタンをタップ」等）。
  多くのステップの `check`/`enter`/`tick` は無変更（`game._paddleBounces`・`game.charge`・
  `game._resumes` 等のカウンタは、入力がタッチ経由でもキーボード経由でも同じように増減するため）。
  ただし**左右移動・前後移動の2ステップは判定ロジック自体もタッチ対応に修正**（Issue #63）:
  タッチのドラッグ操作はパドルの目標位置を直接指定する方式で `keys.left`/`right`/`up`/`down` を
  一切使わないため、これらのキー状態を見ていた旧ロジックのままだとタッチ操作だけでは永久に
  完了できない状態だった。実際のパドル位置の変化（`tick` で前フレームとの差分を見る）で
  判定するよう変更し、キーボード操作でもドラッグ操作でも同じ条件で完了できるようにした。
  前後移動はパドルが最初から一番後ろの位置にいて後ろへ動けないため、「前に動かしてから
  後ろに戻す」動きで両方向を試したとみなす。`Escape`キーでのスキップ表示はタッチ操作の対応が
  無いため、モバイル版のチュートリアル画面からは表示を外している（`skipTutorial()` 自体は
  キーボード用に残っており、Bluetoothキーボード接続時はEscapeで使える）。
