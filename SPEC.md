...
- バージョン表示: `const VERSION = "1.0.<PR番号>";`（`canvas`/`ctx` 取得の直後に定義）を、`draw()` の `ready` 分岐の最後で画面右下（`x = W-8` 右寄せ, `y = H-8`）に `"v" + VERSION` として小さく（11px・グレー`#6c7486`）表示する。GitHub Pagesが最新の変更を反映しているかを見分けるための目印で、ゲームロジックには一切関与しない。PC版・モバイル版それぞれの `game.js` に同じ値を持たせ、変更を含むPRを作成するたびに手動でその**PRの番号**に更新する（マージ前にPR番号が確定してから値を合わせる運用）
...
- 反射角度の自然化（Issue #51、モバイル版のみ）: `reflectBallOffRect(ball, rect)`
  （ブロック・壊れないブロック・ボス・フラフラ障害物などの反射に共通で使われる）は、
  従来は「当たった軸だけを単純に反転する」だけだったが、コーナー以外のヒットでは
  当たった位置に応じて反射角度が変わるように変更した（パドル反射と同じ考え方）。
  - 上下の面（Yヒット）: `hit = clamp((ball.x - rectCenterX) / (rect.w/2 + ball.r), -1, 1)`、
    `angle = hit * CONFIG.brickBounceAngleMax`（既定 `Math.PI/6`＝30度）とし、反転後の
    `dy` の符号（上向き/下向き）は保ったまま `dx = speed * sin(angle)`、
    `dy = sign(dy) * |speed * cos(angle)|` で速さを保存しつつ角度をつける。
  - 左右の面（Xヒット）も同様に、当たった高さ（`ball.y`）に応じて `dy` に角度がつく
    （軸が入れ替わるだけで式の形は同じ）。
  - 角（コーナー）ヒットは従来どおり単純な二軸反転のまま（発生頻度が低く、複雑化を避けた）。
  - PC版（`game.js`）は変更していない。ユーザーが実機で挙動を確認してから方針を決めたいと
    いう意向のため、まずモバイル版にのみ適用している。
